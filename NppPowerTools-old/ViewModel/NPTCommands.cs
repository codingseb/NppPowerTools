...
namespace NppPowerTools
{
    ...
    {
        private static readonly Regex charRegex = new(".", RegexOptions.Compiled);
                ...
                Commands = commandsNodesList
                    ...
                    .Select(node =>
                    {
                        ...
                        return new NPTCommand()
                        {
                            ...
                            CommandAction = win =>
                            {
                                Npp.NotepadPP.CallMenuCommand(nppMenuCmd);
                                win.Close();
                            }
                        };
                    })
                    ...
                    .Append(new NPTCommand()
                    {
                        ...
                        CommandAction = win =>
                        {
                            try
                            {
                                int currentLine = Npp.Scintilla.GetCurrentLineNumber();
                                string line = Npp.GetLineText(currentLine + 1);
                                string text = Npp.Scintilla.GetText(Npp.Scintilla.GetLineEndPosition(currentLine) + 1);
                    ...
                    .Append(new NPTCommand()
                    {
                        ...
                        CommandAction = win =>
                        {
                            bool isSelection = Npp.SelectedText.Length > 0;

                            string text = string.Join("\r\n", (isSelection ? Npp.SelectedText : Npp.Text).Split(new string[] { "\r\n", "\r", "\n" }, StringSplitOptions.None).Reverse());

                            if (isSelection)
                                Npp.SelectedText = text;
                            else
                                Npp.Text = text;
        ...
        {
            NPTCommand CreateLangCommand(string name, LangType langType) => new()
            {
                Name = name,
                ResultOrInfoSup = langType,
                CommandAction = win =>
                {
                    Npp.NotepadPP.SetCurrentLanguage(langType);
                    win.Close();
                }
            };
        ...
        {
            Regex findRegex = new(charRegex.Replace(filter, match => "(?<between>[^" + Regex.Escape(match.Value) + "]*)(?<match>" + Regex.Escape(match.Value) + ")") + "(?<end>.*)", RegexOptions.IgnoreCase);
