...
namespace NppPowerTools.Utils
{
    internal static class CustomEvaluations
    {
        ...
        private static readonly List<IVariableEvaluation> variablesEvaluations = new()
        {
        ...
        private static readonly List<IFunctionEvaluation> functionsEvaluations = new()
        {
            LoopEvaluation.Instance,
            NppAccessEvaluation.Instance,
            JsonEvaluation.Instance,
        ...
        private static readonly List<IIndexingEvaluation> indexingEvaluations = new()
        {
            JsonEvaluation.Instance,
        };
        ...
        public static void Evaluator_EvaluateVariable(object sender, VariableEvaluationEventArg e)
        {
            ...
            {
                e.Value = Npp.NotepadPP.GetCurrentLanguage();
            }
        ...
        public static void Evaluator_EvaluateFunction(object sender, FunctionEvaluationEventArg e)
        {
            ...
            {
                if (e.Args.Count > 1)
                    Print += string.Format(e.EvaluateArg(0).ToString(), e.Args.Skip(1).ToArray()) + Npp.CurrentEOL;
                else
                    Print += e.EvaluateArg(0) + Npp.CurrentEOL;
