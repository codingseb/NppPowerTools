...
namespace NppPowerTools.Utils
{
    ...
    public partial class ExpressionEvaluator
    {
        ...
        protected static readonly Regex varOrFunctionRegEx = new(@"^((?<sign>[+-])|(?<prefixOperator>[+][+]|--)|(?<varKeyword>var)\s+|(?<dynamicKeyword>dynamic)\s+|((?<nullConditional>[?])?(?<inObject>\.))?)(?<name>[\p{L}_](?>[\p{L}_0-9]*))(?>\s*)((?<assignationOperator>(?<assignmentPrefix>[+\-*/%&|^]|<<|>>|\?\?)?=(?![=>]))|(?<postfixOperator>([+][+]|--)(?![\p{L}_0-9]))|((?<isgeneric>[<](?>([\p{L}_](?>[\p{L}_0-9]*)|(?>\s+)|[,\.])+|(?<gentag>[<])|(?<-gentag>[>]))*(?(gentag)(?!))[>])?(?<isfunction>[(])?))", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        protected static readonly Regex indexingBeginningRegex = new(@"^(?<nullConditional>[?])?\[", RegexOptions.Compiled);
        protected static readonly Regex arrayTypeDetectionRegex = new(@"^(\s*(\[(?>(?>\s+)|[,])*)\])+", RegexOptions.Compiled);
        protected static readonly Regex assignationOrPostFixOperatorRegex = new(@"^(?>\s*)((?<assignmentPrefix>[+\-*/%&|^]|<<|>>|\?\?)?=(?![=>])|(?<postfixOperator>([+][+]|--)(?![\p{L}_0-9])))");
        ...
        protected Regex instanceCreationKeywordRegex = new(@"^new(?>\s+)((?<isAnonymous>[{{])|((?<name>[\p{L}_][\p{L}_0-9\.]*)(?>\s*)(?<isgeneric>[<](?>[^<>]+|(?<gentag>[<])|(?<-gentag>[>]))*(?(gentag)(?!))[>])?(?>\s*)((?<isfunction>[(])|(?<isArray>\[)|(?<isInit>[{{]))?))", RegexOptions.Compiled);
        protected Regex initializationPropertyRegex = new(@"^[\p{L}_][\p{L}_0-9]*", RegexOptions.Compiled);
        protected Regex castRegex = new(@"^\((?>\s*)(?<typeName>[\p{L}_][\p{L}_0-9\.\[\]<>]*[?]?)(?>\s*)\)", RegexOptions.Compiled);
        ...
        // For script only
        protected Regex blockKeywordsBeginningRegex = new(@"^(?>\s*)(?<keyword>while|for|foreach|if|else(?>\s*)if|catch)(?>\s*)(?<startBracket>\()", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        protected Regex blockKeywordsHeadAndBodySeparatorRegex = new(@"^(?>\s*)(?<Separator>:)", RegexOptions.Compiled);
        protected Regex blockBeginningRegex = new(@"^(?>\s*)(?<startBracket>[{])", RegexOptions.Compiled);
        protected Regex blockIndentationRegex = new(@"^([\r\n]*(([ ]{4})+.*)([\r\n]|$)+)+", RegexOptions.Compiled);
        protected Regex blockIndentationLevelTrimRegex = new("^[ ]{4}", RegexOptions.Compiled | RegexOptions.Multiline);
        protected Regex nextIsStatementTerminalPunctuatorRegex = new("^(;)", RegexOptions.Compiled);
        protected static readonly Regex wordTypeTokenDetectionRegex = new(@"^(?>[\p{L}_0-9]+)$", RegexOptions.Compiled);
        protected static readonly Regex wordTypeTokenBoundaryValidation = new(@"[^\p{L}_0-9.]", RegexOptions.Compiled);
        ...
        protected static readonly Regex blockKeywordsWithoutHeadStatementBeginningRegex = new(@"^(?>\s*)(?<keyword>else|do|try|finally)(?![\p{L}_0-9])", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        public object Evaluate(string expression)
        {
            ...
            try
            {
                ExpressionEvaluationEventArg expressionEvaluationEventArg = new(expression, this);
                ...
                if (varFuncMatch.Groups["isfunction"].Success)
                {
                            ...
                            .Any(methodInfo => methodInfo.Name.Equals(varFuncName, StringComparisonForCasing)) == true)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                FunctionPreEvaluationEventArg functionPreEvaluationEventArg = new(varFuncName, funcArgs, this, obj, genericsTypes, GetConcreteTypes);
                                ...
                                {
                                    int argIndex = 0;
                                    List<ArgKeywordsEncaps> argsWithKeywords = new();
                                    ...
                                    List<object> oArgs = funcArgs.ConvertAll(arg =>
                                    {
                                        ...
                                        {
                                            ArgKeywordsEncaps argKeywordEncaps = new()
                                            {
                                                Index = argIndex,
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            FunctionEvaluationEventArg functionEvaluationEventArg = new(varFuncName, funcArgs, this, obj ?? keepObj, genericsTypes, GetConcreteTypes);
                    ...
                    {
                        FunctionPreEvaluationEventArg functionPreEvaluationEventArg = new(varFuncName, funcArgs, this, null, genericsTypes, GetConcreteTypes);
                        ...
                        {
                            FunctionEvaluationEventArg functionEvaluationEventArg = new(varFuncName, funcArgs, this, genericTypes: genericsTypes, evaluateGenericTypes: GetConcreteTypes);
        ...
        protected virtual bool EvaluateIndexing(string expression, Stack<object> stack, ref int i)
        {
            ...
            if (indexingBeginningMatch.Success)
            {
                ...
                IndexingPreEvaluationEventArg indexingPreEvaluationEventArg = new(indexingArgs, this, left);
        ...
        protected virtual MethodInfo TryToCastMethodParametersToMakeItCallable(MethodInfo methodInfoToCast, List<object> modifiedArgs, string genericsTypes, Type[] inferedGenericsTypes, object onInstance = null)
        {
            ...
            for (int a = 0; a < modifiedArgs.Count && parametersCastOK; a++)
            {
                ...
                if (modifiedArgs[a] is InternalDelegate internalDelegate)
                {
                    ...
                    {
                        DelegateEncaps de = new(internalDelegate);
                ...
                else
                {
                    ...
                    if (!parametersCastOK)
                    {
                        try
                        {
                            ParameterCastEvaluationEventArg parameterCastEvaluationEventArg = new(methodInfoToCast, parameterType, modifiedArgs[a], a, this, onInstance);
