...
namespace NppPowerTools.Utils
{
    ...
    {
        private static readonly Regex cmdRegex = new(@"^[$]((?<wait>w[ait]?)?(?<process>p)?)*\s*(?<cmd>.*)", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        private object ExecuteCommand(string command, bool wait, bool returnProcess)
        {
            ...
            ProcessStartInfo procStartInfo = new(fileName, args)
            {
